...
#
def read_squad_examples(file_path, is_training):

  input_file = file_path+'train/'
  """Read a SQuAD json file into a list of SquadExample."""
  is_training = True

  with io.open(input_file+'real_con_eng.txt', encoding='utf-8') as source_doc_file:
    source_doc_lines = source_doc_file.readlines()

  with io.open(input_file+'real_que_eng.txt', encoding="utf-8") as source_que_file:
    source_que_lines = source_que_file.readlines()

  with io.open(input_file+'real_ans_eng.txt', encoding="utf-8") as source_ans_file:
    source_ans_lines = source_ans_file.readlines()

  with io.open(input_file+'real_span_eng.txt', encoding="utf-8") as source_span_file:
    source_span_lines = source_span_file.readlines()

  with io.open(input_file+'real_con_tel.txt', encoding="utf-8") as target_doc_file:
    target_doc_lines = target_doc_file.readlines()

  with io.open(input_file+'real_que_tel.txt', encoding="utf-8") as target_que_file:
    target_que_lines = target_que_file.readlines()

  with io.open(input_file+'real_ans_tel.txt', encoding="utf-8") as target_ans_file:
    target_ans_lines = target_ans_file.readlines()

  with io.open(input_file+'real_span_tel.txt', encoding="utf-8") as target_span_file:
    target_span_lines = target_span_file.readlines()    
...
#   training_size = int((len(source_doc_lines)/100)*85)
#   prediction_size = len(source_doc_lines)-training_size
#   print(training_size,prediction_size)

#   train_nums = list(random.sample(range(len(source_doc_lines)), training_size))
#   eval_nums = [int(x) for x in range(len(source_doc_lines)) if x not in train_nums]

  for i in range(len(target_que_lines)):
    qas_id = i
    question_text = target_que_lines[i].strip()
    doc_tokens = word_tokenize(target_doc_lines[i].strip())
    orig_answer_text = target_ans_lines[i].strip()
    start_position = int((target_span_lines[i].strip()).split('\t')[0])
    end_position = int((target_span_lines[i].strip()).split('\t')[1])

    source_question_text = source_que_lines[i].strip()
    source_doc_tokens = (source_doc_lines[i].strip()).split()
    source_orig_answer_text = source_ans_lines[i].strip()
    source_start_position = int((source_span_lines[i].strip()).split('\t')[0])
    source_end_position = int((source_span_lines[i].strip()).split('\t')[1])
  ...
  with io.open(test_file+'real_con_eng.txt', encoding='utf-8') as source_doc_file_test:
    source_doc_lines_test = source_doc_file_test.readlines()

  with io.open(test_file+'real_que_eng.txt', encoding="utf-8") as source_que_file_test:
    source_que_lines_test = source_que_file_test.readlines()

  with io.open(test_file+'real_ans_eng.txt', encoding="utf-8") as source_ans_file_test:
    source_ans_lines_test = source_ans_file_test.readlines()

  with io.open(test_file+'real_span_eng.txt', encoding="utf-8") as source_span_file_test:
    source_span_lines_test = source_span_file_test.readlines()

  with io.open(test_file+'real_con_tel.txt', encoding="utf-8") as target_doc_file_test:
    target_doc_lines_test = target_doc_file_test.readlines()

  with io.open(test_file+'real_que_tel.txt', encoding="utf-8") as target_que_file_test:
    target_que_lines_test = target_que_file_test.readlines()

  with io.open(test_file+'real_ans_tel_c.txt', encoding="utf-8") as target_ans_file_test:
    target_ans_lines_test = target_ans_file_test.readlines()

  with io.open(test_file+'real_span_tel.txt', encoding="utf-8") as target_span_file_test:
    target_span_lines_test = target_span_file_test.readlines()


  for i in range(len(source_doc_lines_test)):
    qas_id = i
    question_text = target_que_lines_test[i].strip()
    doc_tokens = word_tokenize(target_doc_lines_test[i].strip())
    orig_answer_text = target_ans_lines_test[i].strip()
    start_position = int((target_span_lines_test[i].strip()).split('\t')[0])
    end_position = int((target_span_lines_test[i].strip()).split('\t')[1])

    source_question_text = source_que_lines_test[i].strip()
    source_doc_tokens = (source_doc_lines_test[i].strip()).split()
    source_orig_answer_text = source_ans_lines_test[i].strip()
    source_start_position = int((source_span_lines_test[i].strip()).split('\t')[0])
    source_end_position = int((source_span_lines_test[i].strip()).split('\t')[1])
...
def convert_source_examples_to_features(tokenizer, example, is_training):
  ...
  max_tokens_for_doc = FLAGS.max_seq_length - len(query_tokens) - len(ans_tokens) - 4
  ...
  for (doc_span_index, doc_span) in enumerate(doc_spans):
    ...
    if is_training:
      ...
      if out_of_span:
        ...
        doc_offset = len(ans_tokens)+len(query_tokens) + 3
...
#   tokenizer = ChineseFullTokenizer(vocab_file=FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
  ...
  for (example_index, example) in enumerate(examples):
    ...
    ans_tokens = tokenizer.tokenize(example.orig_answer_text)
    ...
    # The -3 accounts for [CLS], [SEP] and [SEP]
    max_tokens_for_doc = max_seq_length - len(query_tokens) - len(ans_tokens) - 4
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    all_predictions[example.qas_id] = str(best_non_null_entry.text) #nbest_json[0]["text"]
...
#   tf.gfile.MakeDirs(FLAGS.output_dir)
  ...
  if FLAGS.do_train:
    train_examples,eval_examples = read_squad_examples(file_path=FLAGS.train_file, is_training=True)
  ...
  tf.logging.info("***** after estimator *****")
